 ...
 *
 * Run directly with: `node --test --experimental-strip-types auth/offlineTokenProvider.spec.ts`
 *
...
/** Shared, valid login options reused (and spread-overridden) by every test case. */
...
/** The token-endpoint URL {@link BASE_OPTIONS} must resolve to (asserted across cases). */
...
/** A single request captured by the fetch stub for later assertion. */
interface RecordedCall {
	/** The URL the stub was called with. */
	url: string;
	/** The request init (method, headers, body) the stub received. */
	init: TokenFetchInit;
	/** The form-encoded body parsed into `URLSearchParams` for convenient field assertions. */
	params: URLSearchParams;
}

/** The fetch stub plus the mutable list of calls it has recorded. */
interface FetchStub {
	/** The injectable fetch implementation to pass as `fetchImpl`. */
	fetchImpl: TokenFetch;
	/** Requests recorded in call order; assertions read this after the code under test runs. */
	calls: RecordedCall[];
}
 ...
 *
 * @param responses - Scripted responses consumed in order; calling more often than scripted throws.
 * @returns A {@link FetchStub} exposing the `fetchImpl` to inject and its recorded `calls`.
 */
...
runTestCase('login validates required options', async (): Promise<void> => {
	const stub: FetchStub = makeFetchStub([]);
...
//
// Before this group existed the provider armed its next refresh only on the success path, so a
// single transient failure ended token renewal for the whole life of the process -- silently,
// because the stale access token keeps working until it expires. The obvious repair (re-arm at the
// existing MIN_REFRESH_DELAY_IN_S floor of 1 s) trades that for a 1 Hz poll per client against a
// realm every call container logs into, so the failure path has its own jittered ladder.
// ---------------------------------------------------------------------------------------------

runTestCase('a failed background refresh re-arms the loop and the next attempt recovers', async (): Promise<void> => {
	const stub: FetchStub = makeFetchStub([
		{ body: { access_token: 'access-1', refresh_token: 'offline-1', expires_in: 31 } },
		{ status: 500, body: 'boom' },
		{ body: { access_token: 'access-2', refresh_token: 'offline-2', expires_in: 31 } }
	...
	try {
		...
		await flushMicrotasks();
		await flushMicrotasks();
		assert.equal(stub.calls.length, 2, 'the first refresh should have been attempted and failed');
		...
		await flushMicrotasks();
		assert.equal(stub.calls.length, 2, 'the retry must not fire before the backoff base elapses');
		...
		await flushMicrotasks();
		await flushMicrotasks();
		assert.equal(stub.calls.length, 3, 'the loop must re-arm after a failed refresh');
		assert.equal(provider.getAccessToken(), 'access-2', 'the recovered refresh must install the new token');
		provider.stop();
	} finally {
		mock.timers.reset();
	}
...
runTestCase('consecutive failures back off exponentially and stop growing at the ceiling', async (): Promise<void> => {
	// One login plus nine failing refreshes: enough to walk past the exponent cap.
	...
	try {
		const provider: OfflineTokenProvider = await login({
			...
			// for that failure count -- which is what makes the ladder observable.
