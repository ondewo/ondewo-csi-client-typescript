...
- **`require_serial: true` on the `markdownlint-cli2` hook is what prevents that.** `.markdownlint-cli2.yaml` sets `globs: ['*.md']`, and markdownlint-cli2 lints those globs **in addition to** the filenames pre-commit passes it. pre-commit splits the file list across parallel workers, so the five markdown files here fork two markdownlint-cli2 processes that both re-lint every top-level `*.md` and both write it back with `fix: true`, each from a stale read. Measured on this tree: the parallel run deleted the `* Tracking API Version [5.4.0] …` bullet and the `## Release ONDEWO CSI Typescript Client 4.0.0` heading, ate the `*` list marker off two more bullets, shortened a `*****************` separator to 16 stars and desynced `RELEASE.md` from `src/RELEASE.md`. The same run serialised is lossless. **Never drop `require_serial`.**
...
```text
Release ONDEWO CSI Typescript Client ${ONDEWO_CSI_VERSION}
```

So the heading of a new entry must read exactly `## Release ONDEWO CSI Typescript Client <version>`. **This wording is
...
```bash
grep -c '^## Release ONDEWO CSI Typescript Client ' RELEASE.md     # must be >= 1 for your new version
```
...
This package publishes as **`@ondewo/csi-client-typescript`**, which is not always the repository name — the JS client
...
```bash
npm view @ondewo/csi-client-typescript versions --json
```
